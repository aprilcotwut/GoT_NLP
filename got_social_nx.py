...
# seasons = ['Season 7']
# df = df.loc[df['Season'].isin(seasons)].reset_index(drop=True)
...
ax = sns.heatmap(cos_df, linewidth=0.5, annot=True)
...
ax = sns.heatmap(filtered_cos_df, linewidth=0.5, annot=True)
bottom, top = ax.get_ylim()
ax.set_ylim(bottom + 0.5, top - 0.5)
